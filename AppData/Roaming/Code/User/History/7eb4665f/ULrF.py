...
subset_data.index = range(1, len(subset_data) + 1)
...
data["Income per Person"] = data["Income"] / (data["Children"] + data["Marital Status"])
...
average_age_by_education = data.groupby("Education")["Age"].mean()
data["Average Age by Education"] = data["Education"].map(average_age_by_education)
