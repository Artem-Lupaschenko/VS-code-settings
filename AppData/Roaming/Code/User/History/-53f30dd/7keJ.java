...
public class Lab1 {
	...
	public static void main(String[] args) {
			...
			// Створення потоків
			Thread threadF1 = new Thread(new Func1("T1", A, B, C, MA, MD, E), "T1");
			Thread threadF2 = new Thread(new Func2("T2", MG, MH, MK, ML), "T2");
			Thread threadF3 = new Thread(new Func3("T3", V, MO, P, MT, MS, R), "T3");

			// Встановлення пріоритетів потоків
			threadF1.setPriority(Thread.MAX_PRIORITY);
			threadF2.setPriority(Thread.NORM_PRIORITY);
			threadF3.setPriority(Thread.MIN_PRIORITY);

			System.out.println("threadF1.getName() " + threadF1.getThreadGroup());
...
class Func1 implements Runnable {
    ...
    public Func1(String name, int stackSize, int processor, int[] A, int[] B, int[] C, int[][] MA, int[][] MD, int[] E) {
        this.name = name;
        this.A = A;
        this.B = B;
        this.C = C;
    ...
    private int[] scalarVectorMultiplication(int[] vector, int[][] matrix) {
        ...
        if (vector.length != rows) {
            throw new IllegalArgumentException("Incompatible vector and matrix dimensions");
        }
...
class Func2 implements Runnable {
    ...
    public void run() {
		...
		// Допоміжний метод для обчислення суми матриць
...
class Func3 implements Runnable {
    ...
    private int[] scalarVectorMultiplication(int[] vector, int[][] matrix) {
		...
		// Допоміжні методи для обчислень
