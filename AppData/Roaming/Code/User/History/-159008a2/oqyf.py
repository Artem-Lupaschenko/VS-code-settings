...
# Знайдемо середні значення віку матерів і батьків
mage = data['mage']
fage = data['fage']
...
if (stats.normaltest(smoke_group).pvalue > alpha and stats.normaltest(no_smoke_group).pvalue > alpha):
    ...
    # Використовуємо ... для даних, які мають нормальний розподіл
    ...
    # Використовуємо тест Манна-Уітні для даних, які не мають нормальний розподіл
...
if (stats.normaltest(mother_height).pvalue > alpha and stats.normaltest(child_length).pvalue > alpha):
    ...
    # Використовуємо ... для даних, які не мають нормальний розподіл
