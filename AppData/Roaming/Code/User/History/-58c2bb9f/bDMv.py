...
sns.lineplot(x='month_day', y='meantemp', data=data.loc['2015'])
sns.lineplot(x='month_day', y='meantemp', data=data.loc['2016'])
plt.xticks(data.loc['2015']['month_day'][::30], rotation=45)
...
plt.figure(figsize=(12, 6))
sns.lineplot(data=average_humidity_by_month)
plt.title('Середнє значення вологості за кожний місяць')
plt.show()
...
plt.figure(figsize=(12, 6))
sns.lineplot(data=average_humidity_by_two_weeks)
plt.title('Середнє значення вологості за кожні два тижні весни та літа 2014 року')
plt.show()
...
rolling_average_humidity_2013 = data.loc['2013']['humidity'].rolling(30, min_periods=0).mean()
