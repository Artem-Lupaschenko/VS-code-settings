...
plt.figure(figsize=(10, 6))
sns.barplot(x='cut', y='depth', hue='color', data=data, ci=None)
...
plt.figure(figsize=(10, 6))
sns.histplot(data, x='depth', hue='cut', bins=20, kde=True, multiple='stack')
plt.title("Гістограма глибини діамантів для кожного класу якості у відсотках")
plt.show()

sns.FacetGrid(data, col='cut', aspect=0.5, height=3).map(sns.histplot, 'depth', bins=10)
plt.show()
...
correlation_coefficient = data[['x', 'y', 'z', 'depth', 'table', 'carrat']].corr()
plt.figure(figsize=(10, 6))
