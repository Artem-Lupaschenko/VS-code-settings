...
class MonitorSync {
	...
	public synchronized void signalSync() {
		F1++;
		System.out.println("Збільшено: " + F1);
		// Перевірити чи досягнута кількість сигналів дорівнює кількості потоків 
		if (F1 == P) {
			// Викликати notifyAll(), щоб розбудити всі потоки
			notifyAll();
			P = 0;
		}
	}
	...
	public synchronized void waitSync() {
		if (F1 != 0) {
			try {
				// Чекати, коли буде досягнуто значення числа потоків
				wait();
				System.out.println("Очікує " + Thread.currentThread().getName());
